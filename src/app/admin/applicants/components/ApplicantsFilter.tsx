        ...
        {(["Pending", "Accepted", "Rejected"] as const).map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap flex items-center gap-2 ${
                isActive 
            ...
            >
              {tab}
              <span className={`px-1.5 py-0.5 rounded-md text-[9px] leading-none ${isActive ? 'bg-white/20 text-white' : (isDarkMode ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-400')}`}>
